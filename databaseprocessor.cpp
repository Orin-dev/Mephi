...
QVector<QStringList> DatabaseProcessor::getMyelogram(int id){
    QVector<QStringList> lst;
    QSqlQuery query;
    query.prepare("SELECT myelogram.[blast_cells], myelogram.[prolymphocytes], myelogram.[lymphocytes], myelogram.[sum_granulocytes] FROM study join myelogram on myelogram.id_study = study.id_study where study.id_study = ?");
...
QVector<QStringList> DatabaseProcessor::getStudy(int id){
 ...
 void DatabaseProcessor::addMyelogram(QStringList values, int id_study){
     QSqlQuery query;
     query.prepare("INSERT INTO myelogram (blast_cells, prolymphocytes, lymphocytes, sum_granulocytes, is_clinical, id_study) VALUES (?, ?, ?, ?, \"FALSE\", ?)"); //id
     for(int i = 0; i < 4; i++)
