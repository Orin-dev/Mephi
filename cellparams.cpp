...
/*QVector<CellProps>* CellParams::fillRegions(Coords upleft, Coords downright, QRgb initColor, QRgb thresh)
{
    CNT=0;
    S=0;
    props->clear();
   ...
   //инициализируем область
    ...
    prop.YC=prop.YP=YP=YC=0;
    prop.contour = new QVector<Coords>();
    prop.color=qRgb(0,0,0);
    QRgb color_thresh=color_thresh;
    QRgb color=qRgb(0,0,0);
    QRgb fill_color=initColor;
    if(qRed(initColor)==0)
        color_thresh=255;
    else color_thresh=qRed(initColor);
   // prop.body= new QVector<Coords>();
 //   cnt.X=0;
  //  cnt.Y=0;
    PixCnt=0;
    ...
    //QVector<Coords>* body;
    //body= new QVector<Coords>();
    for(int i=0; i<downright.X-upleft.X;i++) //imageSize.X-1
    {   for(int j=0; j<upleft.Y-downright.Y;j++)
          {
            color=((img->pixel(i,j)));
            if ((color==color_thresh))
             {
                ...
                int m=0;
                //body->append(Coords(i,j));
                fillPixel(i,j,initColor);
                     while(PixCnt!=0)
                     {
                      if(PixCnt>0)
                      {
                        m=Coormas[--PixCnt]; l=Coormas[--PixCnt]; //
                        if((l<downright.X-upleft.X-1)&&(m<upleft.Y-downright.Y-1))
                            {
                                if ((QColor(img->pixel(l+1,m))==color_thresh))
                                {
                                    fillPixel(l+1,m,fill_color);
                                }
                                //else
                                if ((QColor(img->pixel(l,m+1))==color_thresh))
                                {
                                    fillPixel(l,1+m,fill_color);
                                }
                               // else
                                if((l>0)&&(m>0)&&((l<downright.X-upleft.X-1)&&(m<upleft.Y-downright.Y-1)))
                                    {
                                        if ((QColor(img->pixel(l-1,m))==color_thresh))
                                        {
                                            fillPixel(l-1,m,fill_color);
                     ...
                     if(S<criteria)
                     {
                         ...
                         --CNT;* //УБРАТЬ ПИКСЕЛИ ЭТИХ ЗОН С КАРТИНКИ
                         //for(int i=0;i<body->size();i++)
                      //   img->setPixel((body->at(i).X),(body->at(i).Y),qRgb(0,0,0));
                      }
                      else
                     {
                       // contour = bugContour_color(prop.XP,prop.YP, fill_color);
                       // prop.contour= contour;
                        int left=10000;
                        int right=0;
                        int up=0;
                      ...
                      prop.color=fill_color;
                      prop.downright.X=right;
                      prop.downright.Y=bottom;
                      prop.upleft.X=left;
                      prop.upleft.Y=up;
                      ...
                      contour= new QVector<Coords>();
                      //body= new QVector<Coords>();
                      fill_color=qRgb(qRed(fill_color),qGreen(fill_color)+1,qBlue(fill_color));
                      S=0;
                }
            //else break;
        }
